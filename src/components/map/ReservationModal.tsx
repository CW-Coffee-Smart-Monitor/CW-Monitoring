...
export default function ReservationModal({ table, onClose }: ReservationModalProps) {
  ...
  const [guestName, setGuestName] = useState(auth.currentUser?.displayName || auth.currentUser?.email?.split("@")[0] || "");
  ...
  const hourOptions = Array.from({ length: 24 - minHour }, (_, i) => String(minHour + i).padStart(2, "0"));
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    if (!arrivalHour) {
      setError("Jam kedatangan harus dipilih.");
      return;
    }
    ...
    try {
      ...
      // ✅ HAPUS baris ini — jangan panggil reserveTable() untuk pending
      // reserveTable(table.id, id, guestName.trim(), expiresAt);
      // Biarkan SYNC_RESERVATIONS dari Firestore yang set warna map
      // hanya saat status sudah 'confirmed'
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="relative shrink-0 overflow-hidden bg-[#2a0838] px-5 pb-5 pt-4">
        ...
        <div className="relative z-10 flex items-center gap-3">
          <button onClick={onClose} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10 text-white/70 hover:bg-white/20">
            <ArrowLeft className="h-4 w-4" />
          </button>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#D7851F]">Pesan Meja Sekarang</p>
            <h3 className="text-base font-extrabold text-white">{table.name}</h3>
            <p className="mt-0.5 text-[11px] text-white/50">{todayStr.split("-").reverse().join("/")} · Khusus hari ini</p>
          </div>
        </div>
      </div>
        ...
        {success ? (
          <div className="flex flex-col items-center gap-3 px-5 py-10 text-center">
            ...
            <p className="text-sm text-neutral-500">
              Meja <span className="font-semibold text-[#4B135F]">{table.name}</span> sudah dipesan. Silakan datang tepat waktu.
            </p>
          </div>
        ) : (
          /* ── Form ── */
          <form onSubmit={handleSubmit} className="space-y-4 px-5 pb-6 pt-5">
            ...
            <div>
              ...
              <p className="mt-1.5 text-[11px] text-neutral-400">
                Toleransi keterlambatan: <span className="font-semibold text-[#D7851F]">30 menit</span> — lewat batas, meja dibebaskan otomatis.
              </p>
            </div>
                ...
                {DURATION_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`flex flex-1 flex-col items-center rounded-xl py-3 text-sm transition-all ${
                      duration === opt.value ? "bg-[#4B135F] text-white shadow-lg shadow-[#4B135F]/25" : "bg-neutral-100 text-neutral-600 hover:bg-neutral-200"
                    }`}
                  >
                    <span className="font-bold">{opt.label}</span>
                    <span className={`text-[10px] mt-0.5 ${duration === opt.value ? "text-white/70" : "text-neutral-400"}`}>{opt.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-3 text-xs text-amber-700">
              ...
              <span>
                Meja akan ditandai <strong>Reserved (kuning)</strong> di peta setelah reservasi <strong>dikonfirmasi</strong> oleh pengelola. Jika tidak hadir dalam 30 menit dari jam yang dipilih, reservasi otomatis dibatalkan.
              </span>
            </div>
            ...
            {/* Submit */}
            <button type="submit" disabled={loading} className="w-full rounded-2xl bg-[#4B135F] py-3.5 text-sm font-bold text-white shadow-lg shadow-[#4B135F]/30 transition-all hover:bg-[#3a0f49] active:scale-[0.98] disabled:opacity-60">
              {loading ? "Menyimpan..." : "Konfirmasi Reservasi →"}
            </button>

            {/* Link to full booking form for future dates */}
            <Link href={`/booking/create?blockCode=${blockCode}`} onClick={onClose} className="block text-center text-xs text-neutral-400 underline-offset-2 hover:text-[#4B135F] hover:underline">
