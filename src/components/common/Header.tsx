...
export default function Header() {
  ...
  //  Ambil unreadCount dari hook yang sudah dibuat kemarin
  const { unreadCount } = useReservationNotifications();
  return (
    <div className="sticky top-0 z-40">
        ...
        <div className="hidden items-center gap-6 px-8 py-5 md:flex">
          ...
          <nav className="flex items-center gap-0.5">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`relative rounded-lg px-4 py-2.5 text-sm font-medium transition-colors ${isActive ? "text-white" : "text-white/60 hover:text-white"}`}>
                  {item.label}
                  {isActive && <span className="absolute bottom-0.5 left-4 right-4 h-0.5 rounded-full bg-white" />}
                </Link>
              );
            })}
          </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Avatar */}
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#4B135F] text-xs font-bold text-white shadow-sm">CW</div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <>
            <motion.div key="overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMenuOpen(false)} className="fixed inset-0 z-40 bg-black/30 backdrop-blur-sm" />
            <motion.div
              key="menu"
                ...
                {NAV_ITEMS.map((item, i) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-4 px-5 py-4 transition-colors ${i < NAV_ITEMS.length - 1 ? "border-b border-neutral-100" : ""} ${isActive ? "bg-[#4B135F]/5" : "hover:bg-neutral-50"}`}
                    >
                      <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${isActive ? "bg-[#4B135F] text-white" : "bg-neutral-100 text-neutral-500"}`}>
                        <item.icon className="h-4 w-4" />
                      </div>
                      <div>
                        <p className={`text-sm font-semibold ${isActive ? "text-[#4B135F]" : "text-neutral-800"}`}>{item.label}</p>
