...
import { Radio, MoreVertical, Search, Filter, AlertTriangle, CheckCircle, XCircle, RefreshCw, Pencil, X, Plus } from "lucide-react";
...
function SessionBadge({ status }: { readonly status: SessionStatus }) {
  ...
  if (status === "UPSELL_READY") {
    return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#F5E9C8] text-[#7D5A1B] whitespace-nowrap border border-[#E8D49A]">UPSELL READY</span>;
  }
  if (status === "GHOST") {
    return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-yellow-400 text-yellow-900 whitespace-nowrap">GHOST</span>;
  }
  return null;
}
...
export default function RfidPage() {
  ...
  function forceCheckout(sessionId: number, cardId: string) {
    ...
    setCards((prev) => prev.map((c) => (c.id === cardId ? { ...c, assetStatus: "In Stock", pairingStatus: "Not Paired" } : c)));
    setOpenMenu(null);
  ...
  function verifyPresence(sessionId: number) {
    setSessions((prev) => prev.map((s) => (s.id === sessionId ? { ...s, sessionStatus: null } : s)));
    setOpenMenu(null);
  ...
  function handleSaveUid() {
    if (!editCard) return;
    setCards((prev) => prev.map((c) => (c.id === editCard.id ? { ...c, uid: editUid, pairingStatus: editUid === "--:--:--:--" ? "Unknown" : c.pairingStatus } : c)));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 bg-white rounded-xl border border-neutral-200 overflow-visible">
          <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100">
            ...
            <span className="px-2.5 py-1 rounded-md bg-neutral-100 text-neutral-500 text-xs font-medium tracking-wide">LIVE FEED</span>
          </div>

          <div className="p-4 space-y-3" ref={menuContainerRef}>
            {visibleSessions.length === 0 ? (
              <div className="py-10 text-center text-neutral-400 text-sm">Tidak ada sesi kartu yang aktif.</div>
            ) : (
              visibleSessions.map((session) => (
                <div key={session.id} className="relative bg-neutral-50 rounded-xl border border-neutral-200 px-4 py-3.5 flex items-center gap-3">
                  {/* Left accent for notable sessions */}
                  {session.sessionStatus && <span className="absolute left-0 top-2 bottom-2 w-1 bg-[#4B135F] rounded-full" />}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-neutral-800">
                      Card #{session.cardId} <span className="text-neutral-400 font-normal">→</span> {session.tableName}
                    </p>
                  ...
                  <div className="relative shrink-0">
                    <button onClick={() => setOpenMenu(openMenu === session.id ? null : session.id)} className="p-1.5 rounded-lg hover:bg-neutral-100 transition-colors text-neutral-400">
                    ...
                    {openMenu === session.id && (
                      <div className="absolute right-0 top-full mt-1 w-56 bg-white border border-neutral-200 rounded-xl shadow-xl z-30 py-1 overflow-hidden">
                        <button onClick={() => forceCheckout(session.id, session.cardId)} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors">
                          <XCircle className="w-4 h-4 shrink-0" />
                          Force Check-out
                        </button>
                        <button onClick={() => markTableFaulty(session.id, session.tableName)} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50 transition-colors">
                          <AlertTriangle className="w-4 h-4 shrink-0 text-amber-500" />
                          Mark Table as Faulty
                        </button>
                        <button onClick={() => verifyPresence(session.id)} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50 transition-colors">
                          <CheckCircle className="w-4 h-4 shrink-0 text-green-500" />
                          Verify Presence
                        </button>
                        <button onClick={() => rebootDevice(session.id)} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50 transition-colors">
          ...
          <div className="px-5 pb-5 pt-2">
            <button type="button" onClick={() => router.push("/admin/rfid/active-sessions")} className="w-full py-2.5 bg-[#4B135F] text-white text-sm font-medium rounded-lg hover:bg-[#3a0f49] transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
            ...
            <div className="p-4">
              ...
              <button onClick={() => router.push("/admin/rfid/tambah")} className="w-full text-left rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-4 hover:bg-[#F4F0FA] hover:border-[#C8B4DC] transition-colors group">
                <p className="text-sm font-semibold text-neutral-800 group-hover:text-[#4B135F] transition-colors">Tambah RFID</p>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100">
              ...
              <button onClick={() => setEditCard(null)} className="p-1.5 rounded-lg hover:bg-neutral-100 text-neutral-400 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => setEditCard(null)} className="flex-1 py-2 text-sm border border-neutral-200 rounded-lg text-neutral-600 hover:bg-neutral-50 transition-colors">
                  Batal
                </button>
                <button onClick={handleSaveUid} className="flex-1 py-2 text-sm bg-[#4B135F] text-white rounded-lg hover:bg-[#3a0f49] transition-colors font-medium">
                  Simpan UID
                </button>
              </div>
            </div>
      ...
      {/* ── Toast ── */}
      {toast && <div className="fixed bottom-6 right-6 bg-neutral-800 text-white text-sm px-4 py-3 rounded-xl shadow-2xl z-50 max-w-sm animate-in fade-in slide-in-from-bottom-2">{toast}</div>}
    </div>
  );
}
