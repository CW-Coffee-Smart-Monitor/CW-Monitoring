...
import { Users, AlertTriangle, Zap, Wifi, Armchair } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const selectedTable =
    selectedId != null ? tables.find((t) => t.id === selectedId) : null;
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const data: ReservationAnalysis[] = snapshot.docs.map((doc) => {
          const d = doc.data();
  ...
  const occupancyBars = useMemo(() => {
    return TIME_SLOTS.map((slot) => {
      const count = reservations.filter((res) => {
        ...
        return isActive && hour >= slot.start && hour < slot.end;
      }).length;
            ...
            >
              ...
              {selectedId !== null &&
                (() => {
                  ...
                  const available = !occupied && !reserved;
                  const th = pos.h ?? 43;
                  const popW = 192;
                  const popH = occupied ? 60 : reserved ? 120 : 60;
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-neutral-200 p-5">
            <div className="flex items-center justify-between mb-0.5">
              ...
              <span className="text-[9px] font-semibold uppercase tracking-widest text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded">
                Mock
              </span>
            </div>
            <p className="text-[10px] text-neutral-400 mb-4">
              Aktivitas sesi terkini
            </p>
            <div className="space-y-3">
              {LIVE_TICKER.map((ev) => (
                <div
                  key={`${ev.time}-${ev.table}`}
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-neutral-400">
                      {ev.time} &middot;{" "}
                      {ev.action === "in" ? "Check-in" : "Check-out"} M.
                      {ev.table}
                    </p>
                  </div>
                </div>
              ...
              <table className="w-full table-fixed text-sm">
                ...
                <thead>
                  <tr className="text-neutral-400 uppercase tracking-wider text-[10px]">
                    <th className="text-left pb-3 font-semibold leading-tight">
                      Table
                      <br />
                      ID
                    </th>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-4">
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        bar.value === maxBar ? "#4B135F" : "#D8CCE8",
                    }}
                    onMouseEnter={(e) =>
                      setHoveredBar({
                        label: bar.label,
                  ...
                  {hoveredBar && (
                    <div
                      className="fixed z-50 px-3 py-2 bg-white text-purple text-xs rounded-lg shadow-lg pointer-events-none"
                      style={{
                        left: hoveredBar.x + 10,
                        top: hoveredBar.y - 40,
                      }}
                    >
                      <p className="font-semibold">{hoveredBar.label}:00</p>
          ...
          <div className="bg-white rounded-xl border border-neutral-200 p-5 flex flex-col gap-5">
            <div>
              <p className="font-semibold text-neutral-800 mb-4">
                Area Clustering
              </p>
              <div className="space-y-4">
                <div>
                  ...
                  <div className="h-2 bg-neutral-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[#4B135F] rounded-full"
                      style={{ width: "76%" }}
                    />
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-sm text-neutral-600 mb-7 leading-relaxed">
              Sistem mendeteksi adanya anomali pada Meja {confirmTableId}.
              Apakah pelanggan benar-benar tidak ada di lokasi atau sensor
              ultrasonik tidak berfungsi dengan baik?
            </p>
